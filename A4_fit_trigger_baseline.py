...
def fit_baseline( c_idx, inh_speed, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = 'trigger_%s_baseline' % (inh_speed)
    if check:
        # is it done already
    ...
    model = c.get_trigger_asd_model(XY_pre)
    ...
    # prepare results
    results = core.TriggerModel(model, durations=durations, **kw[inh_speed])
...
for c_idx in range( len(cells) ):
    jobs.append( (fit_baseline, c_idx, 'slow') )
    #jobs.append( (fit_baseline, c_idx, 'fast') )
