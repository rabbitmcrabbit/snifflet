...
def fit_affinity( c_idx, odour_name, inh_speed, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = 'warp_%s_%s' % (inh_speed, odour_name)
    if check:
        # is it done already
...
def fit_concentration( c_idx, odour_conc_idx, inh_speed, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = 'warp_%s_o%d' % (inh_speed, odour_conc_idx)
    if check:
        # is it done already
        if c.can_load_attribute( attr ):
            return True
...
def fit( c, attr, data, inh_speed ):
    ...
    # XY data for each
    XY_first = data_first.get_warp_XY_data( **kw[inh_speed] )
    XY_pre = data_pre.get_warp_XY_data( **kw[inh_speed] )
