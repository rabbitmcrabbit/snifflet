...
# parameters
kw = {}
kw['slow'] = {'binsize_ms':5, 'D_trigger':100, 'N_triggers':4, 'D_spike_history':0}
...
def fit_affinity( c_idx, odour_name, inh_speed, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = 'trigger_%s_%s' % (inh_speed, odour_name)
    if check:
        # is it done already
...
def fit_concentration( c_idx, odour_conc_idx, inh_speed, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = 'trigger_%s_o%d' % (inh_speed, odour_conc_idx)
    if check:
        # is it done already
        if c.can_load_attribute( attr ):
            return True
...
def fit( c, attr, data, inh_speed ):
    ...
    # XY data for each
    XY_first = data_first.get_trigger_XY_data( **kw[inh_speed] )
    XY_pre = data_pre.get_trigger_XY_data( **kw[inh_speed] )
    ...
    print ' - fitting pre'
    model_pre = c.get_trigger_asd_model(XY_pre)
    ...
    print ' - fitting first'
    model_first = c.get_trigger_asd_model(XY_first)
    ...
    # durations for TriggerModel results
    if inh_speed == 'fast':
        durations = [60]
    elif inh_speed == 'slow':
        durations = [150]
    ...
    results = Bunch()
    # prepare results: pre
    results.pre = core.TriggerModel(model_pre, durations=durations, **kw[inh_speed])
    results.pre.N_sniffs = N_sniffs_pre
    # prepare results: first sniff
    results.first = core.TriggerModel(model_first, durations=durations, **kw[inh_speed])
...
for c_idx in range( len(cells) ):
    for o in odours:
        jobs.append( (fit_affinity, c_idx, o, 'slow') )
        #jobs.append( (fit_affinity, c_idx, o, 'fast') )
    for i in range(1, 7):
        jobs.append( (fit_concentration, c_idx, i, 'slow') )
        #jobs.append( (fit_concentration, c_idx, i, 'fast') )
