...
# parameters
kw = {}
kw['biwarp'] = {'binsize_ms':5, 'D_warp_inh':40, 'D_warp_total':120, 'bisplit_ms':100, 'D_spike_history':0}
...
def fit_all_odours_all_sniffs( c_idx, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_noh_all_odours_all_sniffs' % model
    if check:
        return c.can_load_attribute( attr )
    # fit
...
def fit_odour_all_sniffs( c_idx, model, odour_name, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_noh_%s_all_sniffs' % (model, odour_name)
    if check:
...
def fit_odour_first_sniff( c_idx, model, odour_name, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_noh_%s_first_sniff' % (model, odour_name)
    if check:
        if c.can_load_attribute( attr ):
            return True
        trials_to_keep = c.recording.trial_data.odour_name__a == odour_name
...
def fit_oc_first_sniff( c_idx, model, odour_conc_idx, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_noh_o%d_first_sniff' % (model, odour_conc_idx)
    if check:
        # already done
        if c.can_load_attribute( attr ):
            return True
