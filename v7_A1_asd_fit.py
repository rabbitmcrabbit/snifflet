...
def fit_all_odours_all_sniffs( c_idx, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_all_odours_all_sniffs' % model
    if check:
        return c.can_load_attribute( attr )
    # fit
...
def fit_odour_all_sniffs( c_idx, model, odour_name, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_%s_all_sniffs' % (model, odour_name)
    if check:
...
def fit_odour_first_sniff( c_idx, model, odour_name, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_%s_first_sniff' % (model, odour_name)
    if check:
        if c.can_load_attribute( attr ):
            return True
        trials_to_keep = c.recording.trial_data.odour_name__a == odour_name
...
def fit_oc_first_sniff( c_idx, model, odour_conc_idx, check=False ):
    # check if it is done
    c = cells[c_idx]
    attr = '%s_asd_o%d_first_sniff' % (model, odour_conc_idx)
    if check:
        # already done
        if c.can_load_attribute( attr ):
            return True
...
for c_idx in range( len(cells) ):
    ...
    for model in ['biwarp']:
        ...
        #jobs.append( (fit_all_odours_all_sniffs, c_idx, model) )
        for o in odours:
            #jobs.append( (fit_odour_all_sniffs, c_idx, model, o) )
