...
def construct_X_history( y, D_history ):
    ...
    # construct
    T = len(y)
    X_history = np.zeros( (T, D_history), dtype=y.dtype )
...
def construct_X__ith( X__it, D_history, wrap_value=0 ):
    """ Add history. """
    # construct container
    old_shape = X__it.shape
    new_shape = tuple( list(old_shape) + [D_history] )
    X__ith = np.zeros( new_shape, dtype=X__it.dtype )
    # fill it in
    for h in range( D_history ):
        X__ith[ ..., h  ] = np.roll( X__it, h+1, axis=-1 )
    # remove wrap around at beginning
    for h in range( D_history ):
        X__ith[ ..., h, h: ] = wrap_value
    # reverse
    X__ith = X__ith[ ..., ::-1 ]
    return X__ith
...
class SolverFixedTheta( AutoCR ):
    ...
    def solve( self, grid_search_theta=True, verbose=1, callback=None, **kw ):
        ...
        self.solve_theta( verbose=None, grid_search=grid_search_theta, 
    ...
    def calc_posterior( self, xtol=None, ftol_per_obs=None, verbose=1 ):
        ...
        def step_callback( v, assess_convergence=True ):
        ...
        step_callback( v0, assess_convergence=False )
        # solve
        v_hat = v0
        last_LP = -f(v_hat)
        if not len(v_hat) == 0:
            try:
                v_hat = fmin_ncg( f, v_hat, df, fhess=d2f, 
                        disp=False, callback=step_callback, avextol=xtol )
...
class Solver( SolverFixedTheta ):
    ...
    def solve_theta( self, grid_search=False, ftol_per_obs=None, 
            max_iterations=10, verbose=1, callback=None, **kw ):
    ...
    def get_next_theta_n( self, factr=1e10, ftol_per_obs=None ):
        ...
        # reporting during each step
        def step_callback( theta, assess_convergence=True ):
        ...
        try:
            theta = fmin_l_bfgs_b( 
                    ...
                    factr=factr, disp=False, callback=step_callback )[0]
        except ConvergedUpToFtol:
            theta = self.theta
        # save
        g(theta)
    ...
    def R_c_star__fe( C_c_eig, dims_c_star ):
        ...
        return C_c_eig['Q_c_star__ee'][:, dims_c_star].T
    ...
    def LE_jacobian( C_is_diagonal, second_dr_k, 
        ...
        # non-diagonal case
            ...
            # derivatives wrt `theta_h` variables
            for j in range( N_theta ):
                dC__ff = dC_dtheta_plus__iff[ j ]
                B__ff = mdot( Lam_Cinv__ff, dC__ff, Cinv_c_plus__ff )
                Bk__f = ldiv( 
                        Lambdainv_c_plus__ff, 
...
class TriggerASD( Prior ):
    
    C_is_diagonal = False
    R_is_identity = False
    
    # default variables
    hyperparameter_names = [ 'log_delta', 'rho' ]
    bounds_theta = [ (-3, 3), (-15, 15) ]
    default_theta0 = [0., 0.]
    ...
    @cached
    def Delta__dd( data ):
        D = np.empty( (data.D, data.D) )
        i = np.arange( data.D_trigger, dtype=float )
        Di = (i[None, :] - i[:, None]) ** 2
        D.fill(np.inf)
        for i in range( data.N_triggers ):
            D[ i*data.D_trigger : (i+1)*data.D_trigger, 
                    i*data.D_trigger : (i+1)*data.D_trigger ] = Di
        return D

    @cached
    def C__dd( theta, Delta__dd, D, data ):
        """ Prior covariance matrix for `k`. """
        log_delta, rho = theta
        delta = np.exp( log_delta )
        C = np.exp( -Delta__dd / (2. * delta ** 2) - rho )
        D_other = data.D_spike_history + 1
        C[-D_other:, -D_other:] = 100 * np.eye(D_other)
        return C

    @cached
    def dC_dtheta__idd( theta, Delta__dd, C__dd ):
        """ Derivative of the prior covariance matrix. """
        log_delta, rho = theta
        delta = np.exp( log_delta )
        dC_dlogdelta = 1. / (delta ** 2) * ( Delta__dd * C__dd )
        dC_dlogdelta[ ~np.isfinite(dC_dlogdelta) ] = 0
        dC_drho = -C__dd
        return [ dC_dlogdelta, dC_drho ]
       

class FlowASD( TriggerASD ):

    @cached
    def Delta__dd( data ):
        D = np.empty( (data.D, data.D) )
        i = np.arange( data.D_flow, dtype=float )
        Di = (i[None, :] - i[:, None]) ** 2
        D.fill(np.inf)
        D[ :data.D_flow, :data.D_flow ] = Di 
        return D


class WarpASD( TriggerASD ):

    @cached
    def Delta__dd( data ):
        D = np.empty( (data.D, data.D) )
        i = np.arange( data.D_warp_total, dtype=float )
        Di = (i[None, :] - i[:, None]) ** 2
        D.fill(np.inf)
        D[ :data.D_warp_total, :data.D_warp_total ] = Di 
        return D
